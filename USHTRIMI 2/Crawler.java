...
public class Crawler {
	...
	public void crawl() {
		while (!paVizit.isEmpty()) {
			String a = paVizit.remove();               //bej kalimin nga linket e pavizituara tek linket e vizituara 
			tmp.add(a);                                // deri sa te mos kete me linke tek radha e linkeve te pavizituara   
		}

		while (!tmp.isEmpty()) {
			String b = tmp.remove();            //marr elementin e pare qe kam per te vizituar
			if (vizit.contains(b))             // nqs e kam vizituar nje here vazhdoj me elementin tjeter  ne radhe
				continue;
			...
			try {
				...
				Elements linksOnPage = htmlDocument.select("a[href]");    //marr gjithe elementet qe fillojn me atributin  href
				for (Element link : linksOnPage) {
					if (link.absUrl("href").length() == 0)
						continue;
					this.paVizit.add(link.absUrl("href"));         //linket qe gjehen  i vendos tek radha e linkeve te pavizituara
				}
				vizit.add(b);               //pasi kam mbaruar e shtoj linkun qe po vizitoja te linket e vizituara 
				System.out.println(b);
			} catch (IOException ioe) {
				System.out.println("error " + ioe);   //ne rast gabimi printon gabimin

			}
		}
	}

	/*------Metoda set dhe get per te marre radhat-------*/
